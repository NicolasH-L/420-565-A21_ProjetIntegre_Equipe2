...
const StudentNavbar = ({ useStudent }) => {
    ...
    const fetchStudent = async () => {
        const res = await fetch(`http://3.16.148.156:8888/students/get-student/${studentState.matricule}`)
    ...
    const verifyCvValidity = async (matricule) => {
        const res = await fetch('http://3.16.148.156:8888/students/valid-cv/' + matricule)
        return await res.json()
    }
