...
const Contract = ({ currentStatus, contractProp, signature }) => {
    ...
    const verifyPwdStudent = async (matricule, pwd) => {
        const res = await fetch(`http://3.16.148.156:8888/students/verify-password/${matricule}/${pwd}`)
        return await res.json()
    }

    const verifyPwdMonitor = async (email, pwd) => {
        const res = await fetch(`http://3.16.148.156:8888/monitors/verify-password/${email}/${pwd}`)
        return await res.json()
    }

    const verifyPwdAdmin = async (username, pwd) => {
        const res = await fetch(`http://3.16.148.156:8888/admin/verify-password/${username}/${pwd}`)
        return await res.json()
    }
    ...
    const createNotificationForMonitor = async (notification, monitorId) => {
        const result = await fetch(`http://3.16.148.156:8888/notification/save-notification-for-monitor/${monitorId}`,
            {
                method: 'POST',
                headers: {
                    'Content-type': 'application/json'
    ...
    const createNotificationForAdmin = async (notification) => {
        const result = await fetch('http://3.16.148.156:8888/notification/save-notification-for-admin',
            {
                method: 'POST',
                headers: {
                    'Content-type': 'application/json'
