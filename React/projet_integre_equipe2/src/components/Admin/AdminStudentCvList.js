...
const AdminStudentCvList = () => {
    ...
    const fetchDocuments = async (student) => {
        const res = await fetch(`http://3.16.148.156:8888/document/get-all-documents/${student.id}`)
    ...
    const updateCvStatus = async (document, isValid) => {
        const res = await fetch(`http://3.16.148.156:8888/document/update-document/${document.idDocument}/status/${isValid}`,
            {
                method: 'PUT',
                headers: {
    ...
    const createNotificationStudent = async (notification) => {
        const result = await fetch(`http://3.16.148.156:8888/notification/save-notification-for-student/${student.id}`,
            {
                method: 'POST',
                headers: {
                    'Content-type': 'application/json'
