      ...
      <table className="table table-hover bg-light shadow-lg">
        ...
        <tbody>
          <tr
            ...
            :key="student.id"
            :class="`${student.isCvValid ? 'table-success' : 'table-warning'}`"
          >
            ...
            <td className="w-25">
              <button
                ...
                @click="viewStudentCvList(student)"
              >
                Consulter documents
              </button>
              <button
                className="btn btn-success mx-2"
                @click="validateStudent(student)"
              >
                Valider étudiant
              </button>
            </td>
  ...
  methods: {
    ...
    validateStudent(student) {
      AdminService.validateStudent(student).then((response) => {
        this.students = this.students.map((student1) =>
          student1.matricule === student.matricule
            ? { ...student1, isCvValid: response.isCvValid }
            : student1
        );
      });
    },
